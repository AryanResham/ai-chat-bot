...
export default function AuthModal({ type, onClose, onSubmit }) {
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md transform transition-all duration-300 scale-100">
        {/* Header modal name and close button */}
        <div className="relative mb-8">
          ...
          <button
            onClick={onClose}
            className="absolute right-0 top-0 text-slate-400 hover:text-white transition-colors p-1 rounded-lg hover:bg-slate-700"
          >
            <svg
              className="w-6 h-6"
              fill="none"
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            onSubmit(type, new FormData(e.target));
          }}
          className="space-y-4"
        >
          {type === "signup" && (
